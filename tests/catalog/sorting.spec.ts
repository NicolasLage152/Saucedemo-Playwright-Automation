...
test.describe('Sorting Module – Catalog Ordering', () => {
  ...
  test('Validate alphabetical sorting from A to Z (az)', async ({ inventoryPage }) => {
    await inventoryPage.sortProducts('az');
    await expect(inventoryPage.itemNames).toHaveText(NAMES_A_TO_Z);
  });

  test('Validate alphabetical sorting from Z to A (za)', async ({ inventoryPage }) => {
    await inventoryPage.sortProducts('za');
    await expect(inventoryPage.itemNames).toHaveText(NAMES_Z_TO_A);
  });

  test('Validate price sorting from low to high (lohi)', async ({ inventoryPage }) => {
    await inventoryPage.sortProducts('lohi');
    await expect(inventoryPage.itemPrices).toHaveText(PRICES_LOW_TO_HIGH);
  });

  test('Validate price sorting from high to low (hilo)', async ({ inventoryPage }) => {
