...
test.describe("Hamburger Menu – Navigation & Sidebar", () => {
  ...
  test("Validate that the hamburger menu opens and closes correctly", async ({
    ...
    await expect(inventoryPage.navbar.menuWrap).not.toHaveCSS(
      "transform",
      "none",
    );
  });
  ...
  test("Interact with the logo outside the hamburger menu (Negative scenario)", async ({
    page,
    inventoryPage,
  }) => {
    // Click the app logo (a static element in the sidebar header)
    await inventoryPage.navbar.appLogo.click();
    await expect(page).toHaveURL(/.*inventory\.html/);
  });
});
